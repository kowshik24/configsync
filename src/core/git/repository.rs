...
impl GitRepository {
    ...
    pub fn commit_all(&self, message: &str) -> Result<()> {
        ...
        let parent_commit = match self.repo.head() {
            Ok(head) => {
                ...
                Some(self.repo.find_commit(target).context("Failed to find head commit")?)
            }
    ...
    pub fn push(&self) -> Result<()> {
        let mut remote = self.repo.find_remote("origin")
            .context("Failed to find remote 'origin'. If this is a local-only repo, push is not needed.")?;
